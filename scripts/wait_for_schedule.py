...
def dispatch_next(post_id, scheduled_at):
    ...
    request = Request(
        ...
        data=body,
        method="POST",
        headers={"Authorization": f"Bearer {token}", "Accept": "application/vnd.github+json", "Content-Type": "application/json", "User-Agent": "EB-Instagram-Automacao", "X-GitHub-Api-Version": "2022-11-28"},
    )
    try:
...
def main():
    ...
    if not os.getenv("IG_TOKEN") or not os.getenv("IG_USER_ID"):
